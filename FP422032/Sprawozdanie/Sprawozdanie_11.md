...
Napisałem krótki skrypt w Bashu, który sprawdzał, czy wdrożenie nowej wersji zdąży się wykonać w 60 sekund (wykorzystałem komendę `minikube kubectl -- rollout status`). Jeśli pody nie wstałyby w tym czasie (np. przez wadliwy obraz), skrypt zwróciłby błąd. Można to łatwo podpiąć np. pod Jenkinsa, żeby przerywał pipeline w razie problemów na klastrze.
